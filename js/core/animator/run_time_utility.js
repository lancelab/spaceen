...
( function () {
	...
	graph.drawScaledStubsToMaster = function ( inPausePhase, master_screen_scale, turnEnterPassed, ownTicks, drawBeforeSprites )
	{
			...
			if( conf.virt === 'b' || conf.virt === 't' ) //if( master_context !== virt.con[ virt.ix ] )
			{
			...
			if( dsprite.drawPauseImage && turnEnterPassed || ( fillTitleFrom && fillTitleFrom > ownTicks ))
			//inPausePhase > -1 )	//TODM too expensive calls after pause. Rid them.
			{
				dsprite.drawPauseImage (
					inPausePhase,
					master_context,
					ww * 0.5,
					wh * 0.5,
