...
( function ( $ ) {
	...
	graph.init = function ( requested_3D_navigator, foregrounds_ )
	{
			...
			if( conf.bgCnsOn )
			{
				...
				ww.height		= conf.vheight;
				virt.bgCns		= ww;
				virt.bgCtx		= ww.getContext( '2d' );
				virt.bgImg		= virt.bgImg || null;	// readability sugar;
			}
	...
	graph.reset_animation = function ()
	{
			...
			var w_sq = $(window);
			var ww_w = w_sq.width();
			var ww_h = w_sq.height(); // Math.max( d_sq.height(), w_sq.height() );
			...
			if( conf.csize === 'f' )
			{
				//:	TODM do once, not at every resize
				...
				canvas_wrap.style.left		= '0px';	// TODM mess: how come in non-absolute? and non-relative;
			...
			if( graph.startTime !== null )
			{
				if( conf.timeFromTicks ) graph.startTime ? 0 : (new Date()).getTime();
				mstones.ticks		= 0;		//	counts animation phase till the browser's death. Independent from window resizes.
				mstones.pastTicks	= 0;
			}
			*/
